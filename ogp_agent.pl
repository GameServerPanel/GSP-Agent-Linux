...
sub collect_and_insert_process_samples
{
	...
	# Only monitor processes that have corresponding startup files
	# This ensures we only track game servers that are properly configured
	foreach my $proc_pattern (keys %startup_executables) {
		...
		foreach my $pid (@pids) {
			...
			# Use startup info from the corresponding startup file
